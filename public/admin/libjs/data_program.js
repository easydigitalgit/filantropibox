var nama_module = 'data_program';
var label_module = 'Data Program';
...
form_element['input'] = ['#id', '#nama_program', '#mulai', '#berakhir'];
form_element['select'] = ['#kategori'];
form_element['textarea'] = ['#keterangan'];
form_element['imageInput'] = ['#gambar'];
form_element['imagePreview'] = ['.preview_gambar'];
...
//Pilihan
var kategori = ['Pendidikan', 'Ekonomi', 'Kesehatan', 'Lingkungan', 'Kemanusiaan'];
...
function loadTableDataProgram(table) {
...
$(document).on('click', '.addBtn', function (e) {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status) {
                for (let i = 1; i <= kategori.length; i++) {
                    $(form_element['select'][0]).append('<option value="'+i+'">['+i+']\t'+kategori[i-1]+'</option>')
                };
                $('.saveBtn').html('Tambah');
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.status) {
                ...
                $(form_element['input'][1]).val(res.data.nama_program);
                $(form_element['input'][2]).val(res.data.start);
                $(form_element['input'][3]).val(res.data.end);

                $(form_element['textarea'][0]).val(res.data.keterangan_program);

                $(form_element['select'][0]).val(res.data.kategori_id);

                if (res.data.gambar) {
                    $(form_element['imagePreview'][0]).attr('src', base_url + 'images/program/' + res.data.gambar);
                }
